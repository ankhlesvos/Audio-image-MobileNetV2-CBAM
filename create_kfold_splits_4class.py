"""
create_kfold_splits_4class.py

Produces K stratified folds (file-level) from the 4-CLASS DeepShip training pool
(data/deepship_4class/train_list.txt — all unique recording files).

For each fold i in [0, K):
  data/kfold_4class/fold{i}_train.txt  — segments from (K-1) folds
  data/kfold_4class/fold{i}_val.txt    — segments from fold i (held-out)

K=5 → ~10 val files/fold, and each file appears in the val set exactly once.

Usage:
    python3 create_kfold_splits_4class.py [--k 5] [--seed 42]
              [--source data/deepship_4class/train_list.txt]

Labels (4-class DeepShip):
    0 = Cargo
    1 = Passengership
    2 = Tanker
    3 = Tug
...
DEFAULT_SOURCE = "data/deepship_4class/train_list.txt"
KFOLD_DIR     = "data/kfold_4class"

# 4-class label mapping for display
...
def main():
    ...
    print(f"\nStratified {K}-fold split (seed={args.seed}) — 4-CLASS DEEPSHIP")
    print(f"\n{'Class':>6}  {'Name':<16} {'Total':>6}  {'Files/fold (approx)':>22}")
    print("─" * 55)
    ...
    for cls in sorted(label_files):
        ...
        print(f"  {cls:>4}  {name:<16} {len(fids):>6}  {approx:>22.1f}")
